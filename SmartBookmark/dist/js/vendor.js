...
class Store {
    constructor() {
        this.parameters = { items: new Map() };
        this.bookmarks = { items: new Map() };
        this.DataModelVersion = "1.0";
    }
    initializeDefaults() {
        this.parameters.items.set("PM_1", { id: "PM_1", key: "CurrentTabOrigin", value: parameterutils_1.ParameterUtil.PARAM_TYPE_ACTIVE_TAB + parameterutils_1.ParameterUtil.PARAM_TYPE_SEPARATOR + "origin" });
        this.parameters.items.set("PM_2", { id: "PM_2", key: "CurrentTabHost", value: parameterutils_1.ParameterUtil.PARAM_TYPE_ACTIVE_TAB + parameterutils_1.ParameterUtil.PARAM_TYPE_SEPARATOR + "host" });
        this.parameters.items.set("PM_3", { id: "PM_3", key: "SearchText", value: parameterutils_1.ParameterUtil.PARAM_TYPE_JS_VALUE + parameterutils_1.ParameterUtil.PARAM_TYPE_SEPARATOR + "'news on ' + new Date().toString()" });
        this.parameters.items.set("PM_4", { id: "PM_4", key: "DayOfWeek", value: parameterutils_1.ParameterUtil.PARAM_TYPE_JS_VALUE + parameterutils_1.ParameterUtil.PARAM_TYPE_SEPARATOR + "new Date().toString().split(' ')[0] + 'day'" });
        this.parameters.items.set("PM_5", { id: "PM_5", key: "PageTitle", value: parameterutils_1.ParameterUtil.PARAM_TYPE_ACTIVE_TABE_JS_VALUE + parameterutils_1.ParameterUtil.PARAM_TYPE_SEPARATOR + "document.querySelector('title') ? document.querySelector('title').innerText : 'no title';" });
        this.parameters.items.set("PM_6", { id: "PM_6", key: "TenantId", value: parameterutils_1.ParameterUtil.PARAM_TYPE_ACTIVE_TABE_JS_VALUE + parameterutils_1.ParameterUtil.PARAM_TYPE_SEPARATOR + 'document.querySelector("body > table > tbody > tr:nth-child(3) > td:nth-child(2)").textContent.trim()' });
        this.parameters.items.set("PM_7", { id: "PM_7", key: "OrgId", value: parameterutils_1.ParameterUtil.PARAM_TYPE_ACTIVE_TABE_JS_VALUE + parameterutils_1.ParameterUtil.PARAM_TYPE_SEPARATOR + 'document.querySelector("body > table > tbody > tr:nth-child(2) > td:nth-child(2)").textContent.trim()' });
    ...
    getBookmarks() {
        let p = new Promise((resolve, reject) => {
            chrome.storage.sync.get("BookmarkList", (result) => {
                let bookmarks = Store.deserializeBookmarks(result.BookmarkList);
                resolve(bookmarks);
            });
        });
        return p;
    ...
    getParameters() {
        let p = new Promise((resolve, reject) => {
            chrome.storage.sync.get("ParamList", (result) => {
                let parameters = Store.deserializeParameters(result.ParamList);
                resolve(parameters);
            });
        });
        return p;
    ...
    deleteParameter(paremterIdToDelete) {
        return this.getParameters().then(parameters => {
            parameters.items.delete(paremterIdToDelete);
            return this.saveParameters(parameters);
        });
    }
    static serializeBookmarks(item) {
        let serializedItems = "";
        if (item) {
    ...
    static serializeParameters(item) {
        let serializedItems = "";
        if (item) {
            serializedItems = JSON.stringify([...item.items]);
        }
